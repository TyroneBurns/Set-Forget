...
export default async function handler(req, res) {
  try {
    ...
    const tradeRows = await query(`select * from sf_trades where portfolio_id = 'main' order by created_at desc limit 80`);
    ...
    const baseFor = (daysAgo) => {
      ...
      return matches.length
        ? Number(matches[matches.length - 1].equity_gbp)
        : Number(portfolio?.starting_balance_gbp || 0);
    };
