const el = {
  appName: q('#appName'), pairBig: q('#pairBig'), stateBadge: q('#stateBadge'), lastPrice: q('#lastPrice'),
  statusText: q('#statusText'), bullPct: q('#bullPct'), bearPct: q('#bearPct'), chopPct: q('#chopPct'),
  qualityValue: q('#qualityValue'), adaptiveValue: q('#adaptiveValue'), equityStat: q('#equityStat'),
  cashStat: q('#cashStat'), realisedStat: q('#realisedStat'), positionStat: q('#positionStat'),
  dayReturnStat: q('#dayReturnStat'), weekReturnStat: q('#weekReturnStat'), winRateStat: q('#winRateStat'),
  tradeCountStat: q('#tradeCountStat'), marketsList: q('#marketsList'), signalsList: q('#signalsList'),
...
async function refreshAll() {
  try {
    const [stateRes, marketsRes, tradesRes, signalsRes] = await Promise.all([fetch('/api/state'), fetch('/api/markets'), fetch('/api/trades'), fetch('/api/signals')]);
    renderState(await stateRes.json());
    renderMarkets(await marketsRes.json());
...
function renderState(data) {
  const market = data.activeMarket, portfolio = data.portfolio, openPos = data.openPosition, metrics = data.metrics || {};
...
function renderMarkets(rows) {
  ...
  rows.forEach((m) => {
    ...
    item.className = 'item';
    item.innerHTML = `<div class="row"><div><div><strong>${m.pair}</strong></div><div class="small muted">${m.timeframe} • ${m.last_price ? fmtNum(m.last_price) : '–'}</div></div><div class="pill">${m.state}</div></div><div class="row" style="margin-top:10px"><div class="small muted">Quality ${fmtNum(m.quality_score)} • Conf ${fmtNum(m.confidence_pct)}%</div><div class="small">${m.decision}</div></div>`;
...
function renderTrades(rows) {
  ...
  rows.forEach((t) => {
    const item = document.createElement('div');
    item.className = 'item';
    item.innerHTML = `<div class="row"><div><div><strong>${t.type} ${t.side}</strong></div><div class="small muted">${t.pair} • ${t.source} • ${new Date(t.created_at).toLocaleString()}</div></div><div class="pill">${t.exit_price ? fmtNum(t.exit_price) : fmtNum(t.entry_price)}</div></div><div class="small muted" style="margin-top:10px">£${fmtNum(t.notional_gbp || 0)}${t.pnl_gbp != null ? ` • PnL £${fmtNum(t.pnl_gbp)}` : ''}</div>`;
...
function renderSignals(rows) {
  ...
  rows.forEach((s) => {
    const item = document.createElement('div');
    item.className = 'item';
    item.innerHTML = `<div class="row"><div><div><strong>${s.pair}</strong></div><div class="small muted">${new Date(s.created_at).toLocaleString()}</div></div><div class="pill">${s.state}</div></div><div class="row" style="margin-top:10px"><div class="small muted">Q ${fmtNum(s.quality_score)} • Thr ${fmtNum(s.adaptive_threshold)} • Bull ${fmtNum(s.bull_pct)}%</div><div class="small muted">${s.decision}</div></div>`;
